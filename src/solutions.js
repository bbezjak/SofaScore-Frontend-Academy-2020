module.exports = {
  ...
  createObjectWithMethod(value) {
    /*
    TODO prouči razliku između ova 2 načina
    const object = 
      {
        value: value,
        getValue: () => {
  ...
  createEncapsulatedObject() {
    ...
    return ret();*/
    //TODO moram si još malo pojasniti ovu sintaksu return ret()
    //  `return ret` vraća referencu na vanjsku funkciju
    //  `return ret()` izvede vanjsku funkciju i vrati njen return, tj.
    //      kaj god već ret() vraća, u ovom slučaju to je ovaj objekt
  ...
  stringLooselyEqualToTrue() {
    //TODO random ideja, kako zaš točno ovo radi?
    return "1";
  },
   ...
   */
  formatDate(date) {
    //tu postoji fora sa zagradama (baci pogled na ordering)
    return (
  ...
  multiplyArrayByTwoNew(numberArray) {
    let array = [];
    //array.push(...this.multiplyArrayByTwo(numberArray));
    numberArray.forEach((part, index) => {
      array[index] = part * 2;
    });
    return array;
  ...
  classInheritance(callGetName, callGetLanguage) {
    ...
    class Programmer extends Person {
      ...
      getLanguage() {
        return callGetLanguage(this._language);
      }
    }
  ...
  timeoutIncrement(consumer) {
    // inspiration: https://stackoverflow.com/questions/16275664/javascript-async-callback-and-scope
    for (var i = 1; i <= 3; i += 1) {
      (i => {
        setTimeout(() => {
          consumer(i);
